...
/// has its own scale factor. The scale factor is expected to be a f16 value and stored at the end of each packed row.
/// The first byte in the block holds the K-index + 0 and K-index + 4 values.
/// The K-index + 0 value is stored in the lower order part of the byte (low nibble) while
/// the K-index + 4 value is stored in the higher order part (high nibble).
...
///  byte(s28, s24),byte(s29, s25),byte(s30, s26),byte(s31, s27),...,float16(scale)
///
/// @param[in] n_idx      Row index in the RHS matrix (not packed).
/// @param[in] rhs_stride The number of bytes in each row of the RHS matrix (not packed)
///
/// @return the offset in bytes to the RHS matrix (not packed)
size_t kai_get_rhs_offset_rhs_pack_nxk_qsi4c32ps4s0sf16_qsu4c32s16s0_neon(
    size_t n_idx,        //
    size_t rhs_stride);  //
...
///
/// Packed format: K0/K4 nibble pairing with signed two's complement values (-8..7)
/// and fp16 scales stored at the end of the packed row.
///
...
/// the first nibble (K-index + 0). The higher order of the byte holds the second nibble (K-index + 4).
///
