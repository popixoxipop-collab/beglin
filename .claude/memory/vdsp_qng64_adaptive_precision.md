...
- **D-qNg64-16 (2026-09-08 후속)**: 사용자가 선택한 "넓은 real-kernel 재측정" 시도 — 실제로는
  benefit-metric 데이터 0개로 끝남, 대신 진짜 인프라 문제 2개 발견: (1) 기존 attribution 로그
  749줄 중 1줄만 신규 필드(manifest/orig_argmax/threshold) 있음 → p60 이벤트를 현재 바이너리로
  재생성해서 18개 타겟 새로 확보(작동함). (2) `group_by_triple()`의 "첫 줄 우선" 로직이 신규
  데이터를 무시하는 실버그 발견+수정. **가장 중요한 발견: bob에서 다른 세션(`vdsp_engine_main`)의
  `qwen_quantsim4_bin`이 752% CPU/6GB RSS로 이미 무겁게 돌고 있었고, 제 sweep 시도가 그 위에
  얹히며 load average 2.3→18, swap 509MB→4.9GB로 급등** — 이 계정 기존 사고(bob 동시 무거운
  프로세스→재부팅)와 같은 패턴. 제 프로세스는 즉시 종료(로컬+원격 전부), 다른 세션 프로세스는
  안 건드림. 데이터 무결성 확인(부분 push 없음, 승격 파일 그대로). **다음 시도 전 반드시 bob
  `ps aux -m`/`uptime`로 다른 세션 부하부터 확인할 것.** RESULTS.md D-qNg64-16 참고.

- **재시도(D-qNg64-17, 2026-09-09)도 같은 원인으로 중단**: 사용자 지시로 재시도, load average는
  안정화된 것처럼 보였으나(memory-sorted top-8엔 안 잡힘) 백그라운드 모니터가 CPU%>200 필터로
  같은 peer 프로세스(`qwen_quantsim4_bin`, 117분째 실행 중)를 잡아냄 — swap 77%→92%로 실제
  상승 중이었음. 자기 프로세스(local orchestrator + remote qwen_infer_gpu) 즉시 정리, DB/승격
  파일 무변화 확인. **교훈**: 메모리 기준 top-8 체크만으론 지속적 CPU-heavy 프로세스를 놓칠 수
  있음 — 앞으로는 `ps aux | awk '$3+0>200'`(CPU% 기준 전체 스캔)를 병행할 것.
  실측 benefit-metric은 2라운드 연속 0건 — 파이프라인 문제 아니라 순수 외부(peer 세션) 자원경합.
  p60 18-타겟 데이터셋+credential은 그대로 대기 중, peer 작업 끝나면 바로 재개 가능.
