...
## 장기 목표 / 진행중 트랙
- [vdsp_general_serving_engine_goal](vdsp_general_serving_engine_goal.md) — ★★★ 장기목표: vdsp→Apple Silicon GPU+MoE 서빙엔진(MLX/llama.cpp급). MoE-format safetensors 7단계 CLOSED. V5a/V5b/V5c 정확도게이트 PASS. V5c-fused: Bug1/2/3 전부 근본원인 발견+완전해결 — **KILL-GATE 최종 통과**(2026-08-31): eval-count 81→28로 감축(mlx_lm switch_layers.py 참조 gather_qmm 무cross-product화+K/V persistent array slice_update화) 후, Bug1(RoPE freqs_f32가 함수-로컬 스택버퍼를 host-wrap, eval이 함수반환 후로 미뤄지며 use-after-free — pos=0만 항등회전이라 우연히 안전, pos≥1 비결정적 오염)을 영속버퍼 전환으로 완전수정, 진짜 "토큰당 1 eval" 안전하게 달성. 21.78→~52.91 tok/s(+143%, bar 48.34의 109%). V5d/e는 이제 kill-gate 통과 기준 위에서 재스코프 가능
