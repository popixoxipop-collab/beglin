...
## 장기 목표 / 진행중 트랙
- [vdsp_general_serving_engine_goal](vdsp_general_serving_engine_goal.md) — ★★★ 장기목표: vdsp→Apple Silicon GPU+MoE 서빙엔진(MLX/llama.cpp급). MoE-format safetensors 7단계 CLOSED. V5a/V5b/V5c 정확도게이트 PASS. V5c-fused: Bug1/2/3 전부 근본원인 발견+완전해결 — **KILL-GATE 최종 통과**(2026-08-31, 21.78→~52.91 tok/s, bar 48.34의 109%). V5d(배치 B-token GPU decode): 기존 fused 경로에 배치축 B 일반화(gather_qmm expand_dims 버그 발견+수정), B∈{8..64} 전 구간 **정확도 완전통과(flipped=0)**·determinism 확인. F-4(같은 날 후속): mlx_lm의 global sort/unsort(_gather_sort/_scatter_unsort) 구현, 실 27레이어 pass 내 크로스오버 실측(B=1 손해/B≥8 이득) → **B=64 처리량 109.6→224.3 tok/s(+105%), llama.cpp(180.91)의 1.24배로 역전**, 목표(250)의 89.7%. V5e(ragged)는 아직 미착수
